...
private:
  ...
  void save() {
    ...
    __ enter();
    if (_result != noreg) {
      __ push_call_clobbered_registers_except(_result);
    } else {
      __ push_call_clobbered_registers();
    }
  }
  ...
  void restore() {
    ...
    if (_result != noreg) {
      ...
      __ pop_call_clobbered_registers_except(_result);
    } else {
      __ pop_call_clobbered_registers();
    }
    __ leave();
...
void ZBarrierSetAssembler::try_resolve_jobject_in_native(MacroAssembler* masm,
  ...
  // Test for weak tag
  __ andi(tmp, robj, JNIHandles::weak_tag_mask);
  __ bnez(tmp, weak_tagged);

  // Resolve global handle
  __ ld(robj, Address(robj, -JNIHandles::global_tag_value));
  ...
  // Resolve weak handle
  __ ld(robj, Address(robj, -JNIHandles::weak_tag_value));
